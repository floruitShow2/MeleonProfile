...
export const genCascaderOptions = () => {
  const provinces: string[] = Object.keys(DEFAULT_OPTIONS)

  const res: OptionsType[] = []
  provinces.forEach((province) => {
    const citys = DEFAULT_OPTIONS[province]

    const entity: OptionsType = {
      value: province,
      label: province,
      children: Object.keys(citys).map((city) => {
        return {
          value: city,
          label: city,
          children: citys[city].map((area) => ({ label: area, value: area }))
        }
      })
    }

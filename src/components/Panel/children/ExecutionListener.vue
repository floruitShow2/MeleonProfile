<script setup lang="ts">
  ...
  const createElementListener = (listener: Listener) => {
    const bpmnListener = createListener(modeler!.value!, listener, 'ExecutionListener')
  ...
  useElementUpdateListener(() => {
    ...
    listenerEventTypeOptions.value = getListenerEventTypeOptions('ExecutionListener')
    listenerTypeOptions.value = getListenerTypeOptions()
    scopedListenerList = getExExtensionElementsList(scopedBO, 'ExecutionListener')
...
<template>
  <a-collapse-item key="ExecutionListener">
    <template #header>
      <LucideIcon name="BellRing" />{{ $t('ExecutionListener') }}

<script setup lang="ts">
  import { useCronExpression, WeekdayOptions } from '@/shared/hooks'
  ...
  const {
    ...
    yearValue,
    initCronExpression
  } = useCronExpression($props.value, '0 0/30 * * * ?', () => {
    $emits('update:value', cronExpression.value)
    $emits('change', cronExpression.value)
  })

  watch(
    () => $props.value,
    (val) => {
      val !== cronExpression.value && initCronExpression(val)
    }
  )
    ...
    <div class="cron-preview-card">
      ...
      <div class="cron-card_exp-content">
        <a-input
          ...
          readonly
          @focus="activeTab = 'second'"
        />
        <a-input
          class="is-readonly"
          :model-value="minuteValue"
          readonly
          @focus="activeTab = 'minute'"
        />
        <a-input
          class="is-readonly"
          :model-value="hourValue"
          readonly
          @focus="activeTab = 'hour'"
        />
        <a-input
          class="is-readonly"
          :model-value="dayWeekdayValue[0]"
          readonly
          @focus="activeTab = 'day'"
        />
        <a-input
          ...
          readonly
          @focus="activeTab = 'month'"
        />
        <a-input
          class="is-readonly"
          :model-value="dayWeekdayValue[1]"
          readonly
          @focus="activeTab = 'day'"
        />
        <a-input

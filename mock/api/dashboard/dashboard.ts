...
const apis: MockMethod[] = [
  {
    url: '/mock/api/content-data',
    method: 'get',
    response: (): Service.MockServiceResult<Array<{ x: string; y: number }>> => {
      ...
      return {
        Code: 1,
        Message: 'ok',
        ReturnData: getLineData()
      }
    }
  },
  {
    url: '/mock/api/popular/list',
    method: 'get',
    response: (options: Service.MockOption): Service.MockServiceResult<any[]> => {
      const { type = 'text' } = qs.parseUrl(options.url.toString()).query
      const getReturnData = <T>(list: T[]) => {
        return {
          Code: 1,
          Message: 'ok',
          ReturnData: list
      ...
      if (type === 'image') {
        return getReturnData(videoList)
      }
      if (type === 'video') {
        return getReturnData(imageList)
      }
      return getReturnData(textList)
    }
  }
]
